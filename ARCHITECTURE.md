...
- **Session terminal geometry is auto-computed from the largest pane the session ever appears in.** All session recordings use a fixed 28px font (matching plain terminal panes so the visual weight is consistent). The tmux session cols×rows are derived from that font at the *largest* viewport dim the session appears at across all steps — `cols = floor(inner_width / (28px × 0.60))`, `rows = floor(inner_height / (28px × 1.25))` where `inner = viewport − 2×padding`. Using the largest dim prevents tmux from filling smaller-viewport steps with dots (tmux only fills with dots when the session is *smaller* than the client; a larger session shows cleanly in a smaller viewport as a sub-view). Steps with smaller viewports show the leftmost portion of the wider session — content is never falsely wrapped by the shell, and for typical demo commands (short enough to fit the narrower viewport width) nothing is clipped.
